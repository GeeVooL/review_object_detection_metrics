...
import numpy as np
from metrics.evaluators.pascal_voc_evaluator import (calculate_ap_11_point_interp,
                                                     calculate_ap_every_point)
from metrics.tube import Tube
from metrics.utils.enumerators import MethodAveragePrecision
from metrics.utils.read_files import File
...
class TubeEvaluator():
    ...
    def __init__(self,
        ...
        Args:
            ...
            method (MethodAveragePrecision, optional): Recall interpolation method (see metrics.utils.enumerators). Defaults to MethodAveragePrecision.EVERY_POINT_INTERPOLATION.
        """
    ...
    def evaluate(self, thr=0.5):
        ...
        for obj_cls in self._classes:
            ...
            preds_cls = [
                pred for pred in self._predictions if pred.category_id == obj_cls['id']]

            # sort detections by decreasing confidence
            preds_cls = sorted(
                preds_cls, key=lambda tube: tube.confidence, reverse=True)
            ...
            for vid_id in self._videos:
                ...
                preds = [
                    pred for pred in preds_cls if pred.video_id == vid_id['id']]
            ...
            if self._method == MethodAveragePrecision.EVERY_POINT_INTERPOLATION:
                ...
                raise ValueError(
                    f'Invalid interpolation method: {self._method}')
