...
import numpy as np
from metrics.bounding_box import BBFormat
...
def get_coco_summary(groundtruth_bbs, detected_bbs):
    ...
    iou_thresholds = np.linspace(0.5, 0.95, int(
        np.round((0.95 - 0.5) / 0.05)) + 1, endpoint=True)
    ...
    AP = np.mean([x['AP']
                 for k in full for x in full[k] if x['AP'] is not None])
    ...
    APmedium = [x['AP']
                for k in medium for x in medium[k] if x['AP'] is not None]
