...
from metrics.utils.general_utils import convert_box_xywh2xyxy
...
class Tube(object):
    ...
    def __init__(self, category_id: int, video_id: int, track: list, **kwargs: Any) -> None:
        ...
        self.track = {attr: np.array([det[attr]
                                     for det in track]) for attr in track[0]}

        if 'confidence' in self.track.keys():
            self.confidence = self.__compute_tube_confidence(
    ...
    def __str__(self) -> str:
        ...
        s += "{}, ".format("".join(f"{k}={v}" for k,
                           v in self.__kwargs.items()))
        s += "track=[{}]".format(", ".join(f"{k}: {v}" for k,
                                 v in self.track.items()))
        s += ")"
