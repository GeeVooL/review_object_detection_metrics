...
import pandas as pd
import metrics.utils.general_utils as general_utils
import metrics.utils.validations as validations
from metrics.bounding_box import BoundingBox
from metrics.utils.enumerators import BBFormat, BBType, CoordinatesType
...
def cvat2bb(path):
    ...
    for file_path in annotation_files:
        ...
        for image_info in ET.parse(file_path).iter('image'):
            img_size = (int(image_info.attrib['width']), int(
                image_info.attrib['height']))
...
def openimage2bb(annotations_path, images_dir, bb_type=BBType.GROUND_TRUTH):
    ...
    for file_path in annotation_files:
        ...
        for i, row in csv.iterrows():
            ...
            confidence = None if pd.isna(
                row['Confidence']) else float(row['Confidence'])
        ...
        if 'shapes' in json_object:
            ...
            for obj in json_object['shapes']:
                ...
                if x1 is None and y1 is None and x2 is None and y2 is None:
                    continue
                x1, y1, x2, y2 = int(float(x1)), int(
                    float(y1)), int(float(x2)), int(float(y2))
...
def yolo2bb(annotations_path, images_dir, file_obj_names, bb_type=BBType.GROUND_TRUTH):
    ...
    if not os.path.isfile(file_obj_names):
        print(
            f'Warning: File with names of classes {file_obj_names} not found.')
        return ret
    # Load classes
    all_classes = []
...
def df2labelme(symbolDict, dir_image):
    try:
        symbolDict.rename(columns={
            ...
            inplace=True)
        # Get image path
        image_path = general_utils.find_file(
            dir_image, symbolDict['imagePath'][0])
        ...
        if 'XMin' in symbolDict.columns and 'YMin' in symbolDict.columns and 'XMax' in symbolDict.columns and 'YMax' in symbolDict.columns:
            ...
            symbolDict = symbolDict.groupby(
                ['imagePath', 'imageWidth', 'imageHeight', 'imageData'])
            symbolDict = (
